...
class DottedList(DottedCollection, MutableSequence):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, int) or (
            isinstance(index, basestring) and index.isdigit()
        ):
    ...
    def __setitem__(self, index, value):
        if isinstance(index, int) or (
            ...
            raise IndexError("cannot use %s as index in %s" % (index, repr(self.store)))
    ...
    def __delitem__(self, index):
        if isinstance(index, int) or (
            ...
            raise IndexError("cannot delete %s in %s" % (index, repr(self.store)))
...
class DottedDict(DottedCollection, MutableMapping):
    ...
    def __getitem__(self, k):
        ...
        if not isinstance(target, DottedCollection):
            raise KeyError(
                'cannot get "{0}" in "{1}" ({2})'.format(alt_key, my_key, repr(target))
            )
